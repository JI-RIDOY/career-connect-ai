...
const MyJobs = () => {
  ...
  const fetchMyJobs = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://ai-server-6xda.onrender.com/api/jobs/recruiter/${user.uid}`);
  ...
  const handleDelete = async (jobId) => {
    ...
    try {
      const response = await fetch(`https://ai-server-6xda.onrender.com/api/jobs/${jobId}`, {
        method: 'DELETE'
      });
