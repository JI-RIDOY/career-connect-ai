...
const ResumeBuilder = () => {
  ...
  const fetchResumes = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`https://ai-server-1-ckgg.onrender.com/api/resumes/user/${user.uid}`);
  ...
  const saveResume = async () => {
    ...
    try {
      ...
      const url = editingResumeId 
        ? `https://ai-server-1-ckgg.onrender.com/api/resumes/${editingResumeId}`
        : 'https://ai-server-1-ckgg.onrender.com/api/resumes';
  ...
  const deleteResume = async (id, resumeName) => {
    Swal.fire({
      ...
      preConfirm: async () => {
        try {
          const response = await fetch(`https://ai-server-1-ckgg.onrender.com/api/resumes/${id}`, {
            method: 'DELETE'
          });
  ...
  const generatePDF = async () => {
    ...
    try {
      const result = await Swal.fire({
        ...
        showLoaderOnConfirm: true,
        preConfirm: async () => {
          const response = await fetch('https://ai-server-1-ckgg.onrender.com/api/resumes/generate-pdf', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
