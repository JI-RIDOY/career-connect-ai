...
const Messages = () => {
  ...
  const loadConversations = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://ai-server-1-ckgg.onrender.com/api/messages/conversations/${user.uid}`);
  ...
  const loadMessages = async (before = null) => {
    try {
      ...
      const response = await fetch(
        `https://ai-server-1-ckgg.onrender.com/api/messages/conversation/${selectedConversation.conversationId}?${params}`
      );
