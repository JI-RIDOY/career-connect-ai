...
const JobApplications = () => {
  ...
  const fetchJobAndApplications = async () => {
    try {
      ...
      // Fetch job details
      const jobResponse = await fetch(`https://ai-server-1-ckgg.onrender.com/api/jobs/${id}`);
      ...
      if (jobData.success) {
        ...
        const appsResponse = await fetch(
          `https://ai-server-1-ckgg.onrender.com/api/jobs/${id}/applications?recruiterId=${user.uid}`
        );
  ...
  const handleStatusChange = async (applicationId, newStatus) => {
    try {
      const response = await fetch(
        `https://ai-server-1-ckgg.onrender.com/api/jobs/applications/${applicationId}/status`,
        {
          method: 'PUT',
          headers: {
